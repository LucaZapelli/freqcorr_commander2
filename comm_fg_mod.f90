...
contains
  ...
  subroutine cleanup_fg_mod
    ...
    if (allocated(my_residual))    deallocate(my_residual)
    if (allocated(my_inv_N))       deallocate(my_inv_N)
    if (allocated(my_inv_Ns_fcn))  deallocate(my_inv_Ns_fcn)
    if (allocated(all_residuals))  deallocate(all_residuals)
    if (allocated(all_inv_N))      deallocate(all_inv_N)
    if (allocated(all_inv_Ns_fcn)) deallocate(all_inv_Ns_fcn)
    if (allocated(pix_reg))        deallocate(pix_reg)
    if (allocated(s_reg))          deallocate(s_reg)
    if (allocated(amp_reg))        deallocate(amp_reg)
    if (allocated(w_reg))          deallocate(w_reg)
    if (allocated(scale_reg))      deallocate(scale_reg)
    if (allocated(d_reg))          deallocate(d_reg)
    if (allocated(invN_reg))       deallocate(invN_reg)
    if (allocated(invN_regs_fcn))  deallocate(invN_regs_fcn)
    if (allocated(x_def))          deallocate(x_def)
    if (allocated(fg_par_reg))     deallocate(fg_par_reg)
    if (allocated(p_default))      deallocate(p_default)

  end subroutine cleanup_fg_mod


  subroutine sample_fg_pix_response_map(s, residuals_in, inv_N_in, fg_amp_in, fg_param_map, stat, inv_Ns_in_fcn)
    implicit none
    ...
    real(dp), dimension(1:,0:,1:,1:), intent(in),    optional :: inv_Ns_in_fcn
    ...
    if (myid_chain == root) then
       ...
       inv_N     = inv_N_in
       if (freq_corr_noise) inv_N_fcn = inv_Ns_in_fcn
       fg_amp    = fg_amp_in
    end if
 ...
 !         stop
  ...
  subroutine sample_spec_index_single_region(data, inv_N_rms, amp, region, p, p_local, comp, par_map, &
    ...
    integer(i4b)     :: i, j, k, n, pix, s, status, ind(1), band_iter
...
!         & fg_components(comp)%indlabel(p_local) == 'F857') then
  ...
  function lnL_specind_ARS(x)
    ...
    integer(i4b) :: i, j, n, band_iter
    real(dp)     :: contraction_fcn, f_rhs
...
!!$    end if
  ...
  subroutine init_ind(residuals_in, inv_N_in, index_map, fg_amp, inv_Ns_in_fcn)
    implicit none
...
!!$    stop
  ...
  function fg_init_chisq(p)
    ...
    integer(i4b) :: i, j, k, l, npar, band_iter
    real(dp) :: s, signal, chisq, prior
    real(dp) :: s_main_fcn, contraction_fcn
  ...
  subroutine enforce_pos_amp(chaindir, residuals_in, inv_N_in, fg_amp, fg_param_map_in, doburnin, inv_Ns_in_fcn)
    implicit none
    ...
    allocate(residuals(0:npix-1,nmaps,numband))
    allocate(inv_N(0:npix-1,nmaps,numband))
    if (freq_corr_noise .and. present(inv_Ns_in_fcn)) allocate(inv_N_fcn(numband,0:npix-1,nmaps,numband))
    ...
    if (myid_chain == root) then
       ...
       residuals    = residuals_in
       inv_N        = inv_N_in
       if (freq_corr_noise .and. present(inv_Ns_in_fcn)) inv_N_fcn = inv_Ns_in_fcn
       fg_amp0      = fg_amp
       cdir         = chaindir
       ...
       do b = (t-1)*nblock+1, min(t*nblock, nval)
          ...
          do j = 1, numband
             if (.not. freq_corr_noise) then
                ...
                do band_iter = 1, numband
                   inv_N_local(band_iter,j) = inv_N_fcn(band_iter,i,1,j)
                end do
             end if
          end do
          ...
          do j = 1, numband
             if (.not. freq_corr_noise) then
                ...
                contraction_fcn = 0.d0
                do band_iter = 1, numband
                   contraction_fcn = (residuals(i,1,band_iter)-sum(M(band_iter,:)*p(:,1))) * inv_N_fcn(band_iter,i,1,j)
                end do
                chisq0 = chisq0 + (residuals(i,1,j)-sum(M(j,:)*p(:,1))) * contraction_fcn
             end if
          end do
...
!!$                end if
          ...
          do j = 1, numband
             if (.not. freq_corr_noise) then
                ...
                chisq = chisq + (residuals(i,1,j)-sum(M(j,:)*p(:,1))) * contraction_fcn
             end if
          end do
