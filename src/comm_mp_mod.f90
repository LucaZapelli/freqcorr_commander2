...
!       call fit_all_fixed_mono_dipoles(map_id, cmbmap, cmbmaps_lowres, fg_temp_free, fg_temp_free_lowres)
  ...
  subroutine sample_spectral_param_map(s, residuals, inv_N_noise, fg_amp, index_map, stat, inv_Ns_noise)
    implicit none
    ...
    real(dp), dimension(1:,0:,1:,1:), intent(in), optional :: inv_Ns_noise

    call mpi_bcast(13, 1, MPI_INTEGER, root, comm_chain, ierr)
    if (present(inv_Ns_noise)) then
       call sample_fg_pix_response_map(s, residuals, inv_N_noise, fg_amp, index_map, stat, inv_Ns_noise)
    else
...
!    call init_ind_co(residuals, inv_N_noise, index_map)
  ...
  subroutine init_ind_by_nonlin_search(residuals, inv_N_noise, index_map, fg_amp, inv_Ns_noise)
    implicit none
    ...
    if (present(inv_Ns_noise)) then
       call init_ind(residuals, inv_N_noise, index_map, fg_amp, inv_Ns_noise)
    else
...
!!$  end subroutine get_masked_region_alms
  ...
  subroutine enforce_pos_amps(chaindir, residuals, inv_N_noise, fg_amp, index_map, burnin, inv_Ns_noise) 
    implicit none
    ...
    if (present(inv_Ns_noise)) then
       call enforce_pos_amp(chaindir, residuals, inv_N_noise, fg_amp, index_map, burnin, inv_Ns_noise)
    else
  ...
  subroutine output_ml_map(paramfile, main_band_id)
    implicit none
    ...
    call output_ml_map_engine(myid_chain, paramfile, main_band_id)
