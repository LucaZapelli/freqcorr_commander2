...
contains
  ...
  subroutine initialize_data_mod(paramfile, comm_chain_in, comm_data_in, comm_alms_in, &
    ...
    integer(i4b)       :: ordering, temp_i, num_md_temp, i, j, k, l, ierr, ind, band_iter
    ...
    character(len=2)   :: i2
    character(len=3)   :: i3
    character(len=2)   :: map_text, map2_text, temp_text
    ...
    character(len=128), allocatable, dimension(:,:)   :: temp_names_fcn
    ...
    call mpi_bcast(freq_corr_noise,     1,   MPI_LOGICAL,   root, comm_alms, ierr)    
    ...
    if (freq_corr_noise) then
        allocate(cmbmaps_fcn(numband, 0:map_size-1, nmaps))
        allocate(residuals_fcn(numband, 0:map_size-1, nmaps))
        !allocate(foreground_map(0:map_size-1, nmaps))
        allocate(mask_calibs_fcn(numband, 0:map_size-1, nmaps))
    end if
    ...
    if (sample_fg_pix) then
       allocate(fg_pix_spec_response(0:map_size-1, nmaps, num_fg_comp))
       if (freq_corr_noise) then
          allocate(fg_pix_spec_responses_fcn(numband, 0:map_size-1, nmaps, num_fg_comp))
       end if
    end if
    ...
    if (myid_chain == root) then
       ...
       if (freq_corr_noise) then
          allocate(inv_Ns_scaled_fcn(numband, 0:npix-1, nmaps, numband))
          allocate(inv_Ns_lowres_fcn(numband, 0:npix-1, nmaps, numband))
       end if
  ...
  subroutine read_map_realization(paramfile, realization_id)
    ...
    integer(i4b)       :: ierr, i, j, k, band_iter, map_id_fcn
...
!!$    end if
  ...
  subroutine cleanup_data_mod
    ...
    deallocate(pixels)
    if (allocated(tempamp))                   deallocate(tempamp)
    if (allocated(mask_lowres))               deallocate(mask_lowres)              
    if (allocated(mask_corr))                 deallocate(mask_corr)             
    if (allocated(cmbmaps_fcn))               deallocate(cmbmaps_fcn)
    if (allocated(residuals_fcn))             deallocate(residuals_fcn)
    if (allocated(fg_temps_fcn))              deallocate(fg_temps_fcn)
    if (allocated(fg_pix_spec_responses_fcn)) deallocate(fg_pix_spec_responses_fcn)
    if (allocated(inv_Ns_lowres_fcn))         deallocate(inv_Ns_lowres_fcn)
    if (allocated(inv_Ns_scaled_fcn))         deallocate(inv_Ns_scaled_fcn)
    if (allocated(mask_calibs_fcn))           deallocate(mask_calibs_fcn)
...
!!$    end if
  ...
  subroutine compute_residual(s, subtract_signal)
    ...
    integer(i4b) :: i, j, ierr, band_iter
    ...
    if (output_cmb_freq_map) then
       ...
       if (num_fg_temp > 0) then
          do j = 1, num_fg_temp
             if (.not. freq_corr_noise) then
                ...
                signal = signal + s%temp_amp(j,map_id) * fg_temps_fcn(map_id,:,:,j)
             end if
          end do
       end if
       ...
       do k = 1, num_fg_signal
          ...
          if (.not. freq_corr_noise) then
             ...
             signal = signal + s%fg_amp(pixels,:,k) * fg_pix_spec_responses_fcn(map_id,:,:,k)
          end if
       end do
       ...
       if (.not. freq_corr_noise) then
          ...
          signal = cmbmaps_fcn(map_id,:,:) - signal
       end if
    ...
    do c = 1, num_fg_comp
       ...
       if (fg_components(c)%output_freq_comp_maps) then
          ...
          if (.not. freq_corr_noise) then
             ...
             outmap(pixels,:) = s%fg_amp(pixels,:,c) * fg_pix_spec_responses_fcn(map_id,:,:,c) !* ant2data(map_id)
          end if
          call mpi_reduce(outmap, outmap2, size(outmap), MPI_DOUBLE_PRECISION, MPI_SUM, root, comm_alms, ierr)
          if (myid_alms == root) then
             call int2string(map_id, band)
  ...
  subroutine output_mixing_matrix(map_id, chain_in, iter_in, chain_dir)
    ...
    do i = 1, num_fg_comp
       ...
       if (.not. freq_corr_noise) then
          ...
          outmap(pixels,:) = fg_pix_spec_responses_fcn(map_id,:,:,i)
       end if
       call mpi_reduce(outmap, outmap2, size(outmap), MPI_DOUBLE_PRECISION, MPI_SUM, root, comm_alms, ierr)
       if (myid_alms == root) then
    ...
    if (.not. enforce_zero_cl) then
       ...
       do l = 2, lmax
          if (.not. freq_corr_noise) then
             ...
             alms_cmb(1,l,:) = alms_cmb(1,l,:) * beams_fcn(map_id,l,1)
          end if
       end do
       if (nmaps == 1) then
