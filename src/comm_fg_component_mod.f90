...
!    stop
  ...
  subroutine cleanup_comm_fg_component_mod
    ...
    if (allocated(fg_components)) deallocate(fg_components)
...
!!$  end subroutine compute_eff_power_law_spectrum
  ...
  subroutine update_fg_pix_response_map(band, pixels, fg_pix_spec_response, fg_pix_spec_responses_fcn, fg_param_map_in)
    implicit none

    integer(i4b),                         intent(in)            :: band
    integer(i4b), dimension(0:),          intent(in)            :: pixels
    real(dp),     dimension(0:,1:,1:),    intent(out)           :: fg_pix_spec_response
    real(dp),     dimension(1:,0:,1:,1:), intent(out), optional :: fg_pix_spec_responses_fcn
    real(dp),     dimension(0:,1:,1:),    intent(in),  optional :: fg_param_map_in
 
    integer(i4b) :: i, j, k, p, numpix_highres, pix_nest, pix_ring, ierr, band_iter
             ...
             if ((j == 1 .and. .not. sample_T_modes)) then
                if (.not. freq_corr_noise) then
                   ...
                   do band_iter = 1, numband
                      fg_pix_spec_responses_fcn(band_iter,i,j,k) = 0.d0
                   end do
                end if
             else
                if (.not. freq_corr_noise) then
                   ...
                   do band_iter = 1, numband
                      fg_pix_spec_responses_fcn(band_iter,i,j,k) = &
                           & get_effective_fg_spectrum(fg_components(k), band_iter, fg_par%comp(k)%p(j,:), &
                           & pixel=i, pol=j)
                   end do
                end if
             end if
