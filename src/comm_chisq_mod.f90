...
contains
  ...
  subroutine cleanup_chisq_mod
    implicit none
   
    if (allocated(g_gauss))        deallocate(g_gauss)
    if (allocated(procmask))       deallocate(procmask)
    if (allocated(procmask_full))  deallocate(procmask_full)

  end subroutine cleanup_chisq_mod
  ...
  subroutine compute_total_chisq(map_id, s, output_stats, index_map, chisq_map, chisq_highlat, chisq_fullsky, &
    ...
    integer(i4b) :: i, j, k, l, m, p, ierr, c, testpix(4), band_iter
    ...
    real(dp),     allocatable, dimension(:,:)   :: invN_residual1_dummy_fcn, invN_residual2_dummy_fcn
    real(dp),     allocatable, dimension(:,:,:) :: residuals_fcn_local
    ...
    if (.not. freq_corr_noise) then
       ...
       allocate(invN_residual1_dummy_fcn(0:map_size-1,nmaps))
       allocate(invN_residual2_dummy_fcn(0:map_size-1,nmaps))
    end if
    ...
    do i = 0, map_size-1
       if (.not. freq_corr_noise) then
          ...
          do band_iter = 1, numband
             residuals_fcn_local(band_iter,i,:) = cmbmaps_fcn(band_iter,i,:) - my_signals_fcn(band_iter,i,:)
          end do
       end if
    end do
    !residual = residual
...
!            & sum(residual(i,:)*invN_residual2(i,:))
    ...
    if (output_chisq_band) then
       ...
       if (.not. freq_corr_noise) then
          ...
          my_chisq_band(map_id) = sum(residuals_fcn_local(map_id,:,:)*invN_residual1 * procmask)
       end if
    ...
    if (output_chisq_band_fullsky) then
       ...
       if (.not. freq_corr_noise) then
          ...
          my_chisq_band(map_id) = sum(residuals_fcn_local(map_id,:,:)*invN_residual1 * procmask) + &
               & sum(residuals_fcn_local(map_id,:,:)*invN_residual2 * procmask)
       end if
       call mpi_reduce(my_chisq_band, buffer, numband, MPI_DOUBLE_PRECISION, MPI_SUM, root, comm_chain, ierr)
    ...
    if (output_stats_ .and. output_band_chisq .and. output_chisq) then
       ...
       if (.not. freq_corr_noise) then
          ...
          outmap(pixels,:) = residuals_fcn_local(map_id,:,:)
       end if
...
!            & MPI_SUM, root, comm_chain, ierr)
       if (.not. freq_corr_noise) then
          ...
          call mpi_reduce(sum(residuals_fcn_local(map_id,:,:)*invN_residual1 * procmask), chisq_tot, 1, MPI_DOUBLE_PRECISION, &
               & MPI_SUM, root, comm_chain, ierr)
       end if
    ...
    if (.not. freq_corr_noise) then
       ...
       deallocate(residuals_fcn_local)
       deallocate(invN_residual1_dummy_fcn)
       deallocate(invN_residual2_dummy_fcn)
       deallocate(my_signals_fcn)
    end if
  ...
  subroutine sample_gain(handle, map_id, s, chain, iter)
    ...
    allocate(invN_signal(0:map_size-1,nmaps))
       ...
       if (myid_alms == root) then
          if (g_gauss(map_id,2) == 0.d0) then
             ...
             if (.not. freq_corr_noise) then
                ...
                do i = 1, numval
                   do j = i+1, numval
                      slope(k) = (residuals_fcn_local(map_id,mask2map(i),1)-residuals_fcn_local(map_id,mask2map(j),1)) / &
                           & (my_signals_fcn(map_id,mask2map(i),1)-my_signals_fcn(map_id,mask2map(j),1))
                      k        = k+1
                   end do
                end do
             end if
...
!!$    end if
    ...
    deallocate(invN_signal, my_gain)
    if (.not. freq_corr_noise) then
       deallocate(residual, my_signal)
    else
       deallocate(residuals_fcn_local, my_signals_fcn)
    end if
