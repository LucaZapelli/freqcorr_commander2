...
contains
  ...
  subroutine cleanup_global_par_mod
    implicit none
  
    if (allocated(index_map_rms)) deallocate(index_map_rms)
    if (allocated(x_init)) deallocate(x_init)
 
  end subroutine cleanup_global_par_mod              

  subroutine sample_global_fg_par(handle, s, residuals, inv_N, fg_param_map, stat, inv_Ns_fcn)
    implicit none
    ...
    real(dp), dimension(1:,0:,1:,1:), intent(in), optional :: inv_Ns_fcn

    ! Sample parameter RMS's
    call sample_par_rms(handle, s, fg_param_map, stat)

    ! Optimize priors
    if (present(inv_Ns_fcn)) then
       call optimize_priors(handle, s, residuals, inv_N, fg_param_map, stat, inv_Ns_fcn)
    else
  ...
  subroutine optimize_priors(handle, s, residuals, inv_N, fg_param_map, stat, inv_Ns_fcn)
    implicit none
       ...
       do j = 1, fg_components(i)%npar
          ...
          do l = 1, nstep
             ...
             do q = 1, nsub
                ...
                if (present(inv_Ns_fcn)) then
                   call sample_spectral_param_map(s, residuals, inv_N, s%fg_amp, fg_param_map, stat, inv_Ns_fcn)
                else
                ...
                ! Sample amplitudes
                if (present(inv_Ns_fcn)) then
                   call enforce_pos_amps(chaindir, residuals, inv_N, s%fg_amp, fg_param_map, .false., inv_Ns_fcn)
                else
