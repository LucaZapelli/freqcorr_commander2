program commander
  ...
  integer(i4b)        :: i, j, k, l, m, s, lmax_sht, nside_sht, nmaps_sht, band_iter
...
!  call initialize_cgd_constraint_mod(paramfile, rng_handle, comm_chain, comm_data, comm_alms)
  ...
  call cleanup_beam_mod
  call cleanup_bp_mod
  call cleanup_precond_mod
  call cleanup_chisq_mod
  call cleanup_Cl_util_mod
  call cleanup_data_mod
  call cleanup_comm_fg_component_mod
  call cleanup_fg_mod
  call cleanup_genvec_mod
  call cleanup_global_par_mod
  call cleanup_noiseamp_mod
  call cleanup_utils
  ...
  if (allocated(fg_param_map)) deallocate(fg_param_map)
...
contains
  ...
  subroutine compute_single_chain(paramfile, chain, rng_handle, cl_i)
    ...
    integer(i4b)       :: iter, i, j, k, l, m, num_gibbs_iter, num_step_ml_search, band_iter
    ...
    logical(lgt)       :: output_ml_map_and_covmat, freq_corr_noise
...
!    stop
    if (output_ml_map_and_covmat) then
       ...
       if (.not. freq_corr_noise) then
          ...
          call output_ml_map(paramfile, band_id)
       end if
       return
    end if
...
!       write(58,*)
  ...
  subroutine output_pixel_to_file(chain_dir, chain, objname, pixel, iter, data, invN, cmb_pix, &
       & temp_amp, fg_temp, fg_amp, fg_param, chisq, invNs_fcn)
    implicit none
    ...
    real(dp),         dimension(:,:,:), intent(in), optional :: invNs_fcn

    real(dp)     :: f, scale
    integer(i4b) :: i, j, k, q, unit, band_iter
